...
const PrescriptionForm = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`${process.env.REACT_APP_API_URL}/doctor/getId`, { appointmentId }, { withCredentials: true })

            const patientId = res.data?.patientId

            let data = prescriptions.filter(p => p.drugName.trim() !== '');

            axios.post(`${process.env.REACT_APP_API_URL}/doctor/submit-prescription`, { data: data, notes: note, appointmentId: appointmentId, patientId: patientId, doctorId: patientData?.doctorId }, { withCredentials: true }).then((res) => {
