...
export const PatientSignup = createAsyncThunk(
  ...
  async (formData, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${API_URL}/auth/patient-signup`, formData, {
...
export const fetchUserData = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${API_URL}/auth/getdata`, {}, { withCredentials: true });
...
export const fetchUserList = createAsyncThunk("/auth/fetchUserList", async () => {
  try {
    const response = await axios.post(`${API_URL}/auth/fetchuserlist`, {}, { withCredentials: true })
...
export const updateUserData = createAsyncThunk("auth/updateUserData", async (updatedData) => {
  try {
    const response = await axios.post(`${API_URL}/patient/updatePatientprofile`, updatedData, { withCredentials: true });
    return response.data;
  } catch (error) {
    if (error.response.data.message === "Unauthorized: No token provided") {
      window.location.href = "/login"
...
export const checkUser = createAsyncThunk("auth/checkUser", async () => {
  try {
    const response = await axios.post(`${API_URL}/auth/check`, {}, { withCredentials: true })
    if (response) {
      return true
    }
    else {
